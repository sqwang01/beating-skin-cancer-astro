...
- `src/data/navigator/medicalRules.ts` holds the T-category / stage-grouping tables. As of 2026-09-07 they are **approved** (reviewed by Steven Q. Wang, MD against AJCC 8th; `status: 'approved'`, `STAGING_RULES_ENABLED = true`) for a narrow use: `estimateStageGroup()` computes an **educational Stage I/II sub-group (IA–IIC)** for a node-negative / node-not-needed case, shown on the Step 2 summary via `summary.stageEstimate` + `renderStageEstimate()` in `flow.js`, always labelled an estimate the physician confirms and never overwriting a doctor-reported stage. Everything else — a positive node (III), distant spread (IV), the III/IV sub-groups, clinical-vs-pathologic, any missing pathology value — still defers to the coarse worded `stageBand` and an "ask your physician" message. Do **not** widen the computed scope, and roll the switch back to `false` / `status: 'draft'` if a newer AJCC edition supersedes the 8th or the review lapses.
- **No standalone "T — your original tumor" screen** (removed 2026-09-07, Dr. Wang): the `k0` recap and the summary's `stageEstimate` (IA–IIC table + T explainer) already carry what Breslow thickness and ulceration do, so Step 2 goes straight from the doctor-stage anchor (`k1`) to the `k2` bridge. The early-exit routing hook that used to live on `T1` now lives on `k2`.
- **"Yes, a doctor told me my stage" early exit** (product decision 2026-09-07, Dr. Wang): answering **"Yes"** on `k1` ("Has a doctor already told you your melanoma stage?") routes straight to the "Your stage picture" summary — that clinician-assigned stage is deferred to and the Navigator asks nothing further. The old `k1a` ("what stage were you told?") and `k1b` ("clinical or pathologic?") screens were **deleted**, along with `STEP2_CONSISTENCY_RULES` / the summary `consistencyNote` (every rule compared against `k1a`) and the `k1a`-keyed `stageBand` rule and `k2.autoRouteByTCat` `unless` guards. **"No" / "I'm not sure"** still route to `k2` and walk the full N/M flow. The summary still shows the coarse band / educational estimate from the Step 1 pathology answers, always labelled an estimate the physician confirms.
- **Early exits** (product decision 2026-09-07, Dr. Wang; STEP2 spec §16): an invasive melanoma whose recap/cold-capture has a Breslow thickness **and** ulceration status resolves to a T category and skips the **`k2` bridge** along with the N and M screens, routing straight to the "Your stage picture" summary (`k2` still shows in full for every invasive case that does *not* early-exit). Routing is data (`step2Staging.ts` `k2.autoRouteByTCat`) resolved in `flow.js` `resolveComputedRoute()` via the approved `tCategoryFor()` — evaluated on **entry** to `k2` (so the screen never paints) as well as on its Continue button; it picks a route, computes no stage. Two routes:
  - **T1a** (Breslow **< 0.8 mm**, **no ulceration**): `record`s `{ T1a: 'seen' }`, carries three T1a doctor questions. Summary reuses `estimateStageGroup()` with slnb = not-needed → `confirmed` **Stage IA** and shows the T1a `summary.stageEstimate.caveats` entry (near the 0.8 mm cutoff or with a transected biopsy base a negative SLNB keeps it IA, a positive moves it to III).
  - **T2a–T4b** (Stage IB / IIA / IIB / IIC): `record`s `{ earlyStage: 'seen' }`, carries four doctor questions. A sentinel node biopsy is still expected, so the summary calls `estimateStageGroup()` with slnb left **unknown** → `provisional` **Stage IB/IIA/IIB/IIC** and shows the `earlyStage` `caveats` entry: negative sentinel node → same stage, positive → **Stage III**, distant spread on imaging → **Stage IV**. `copy.provisional` also carries the → IV clause.

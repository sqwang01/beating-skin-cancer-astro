...
- **T1a early exit** (product decision 2026-09-07, Dr. Wang; STEP2 spec §16): an invasive melanoma with Breslow **< 0.8 mm** and **no ulceration** is T1a → Step 2 shows the recap and doctor-stage anchor, then skips the **T education screen** (`T1`) along with the N and M screens and routes straight to the "Your stage picture" summary (no standalone T1a screen; `T1`'s IA–IIC table and T explainer are reproduced on the summary — it still shows in full for every non-T1a invasive case). Routing is data (`step2Staging.ts` `T1.autoRouteByTCat`) resolved in `flow.js` `resolveComputedRoute()` via the approved `tCategoryFor()` — evaluated on **entry** to `T1` (so the screen never paints) as well as on its Continue button; it picks a route, computes no stage. The route `record`s `{ T1a: 'seen' }` and carries the three T1a doctor questions on its `questions`; the summary then reuses `estimateStageGroup()` (slnb = not-needed → `confirmed` Stage IA) and shows `summary.stageEstimate.caveat` (near the 0.8 mm cutoff or with a transected biopsy base a negative SLNB keeps it IA, a positive moves it to III). Suppressed when a doctor reported Stage II+.
